...
def process_video(video_path, model, processor, device, text_features, frame_rate=1, output_dir="results"):
    ...
    output_csv = f"{output_dir}/{video_name}_emotion.csv"
    ...
    for frame_num in range(0, total_frames, frame_rate):
        ...
        similarity_scores = (image_features @ text_features.T).softmax(dim=-1).cpu().numpy()[0]
...
def main():
    ...
    for video_path in tqdm(videos):
        process_video(video_path, model, processor, device, text_features, args.frame_rate, args.output_dir)
