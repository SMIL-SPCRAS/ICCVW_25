...
from training.train_utils_multimodal import (
...
def main():
    # Load config
    base_config = ConfigLoader("config_multimodal.toml")
    ...
    shutil.copy("config_multimodal.toml", os.path.join(results_dir, "config_copy.toml"))
    # File where greedy search will write results
    ...
    for dataset_name in base_config.datasets:
        if dataset_name in ["affwild2", "afew"]:
            ...
            if os.path.exists(base_config.datasets[dataset_name]["csv_path"].format(base_dir=base_config.datasets[dataset_name]["base_dir"], task=base_config.datasets[dataset_name]["task"], split="test", segment=base_config.datasets[dataset_name]["segment"])):
